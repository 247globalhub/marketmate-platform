...
func (h *OrderWebhookHandler) TemuAfterSalesWebhook(c *gin.Context) {
	...
	// Decrypt eventData using AES/CBC with app_secret
	decrypted, err := temuAESDecrypt(payload.EventData, appSecret)
	if err != nil {
		log.Printf("[Temu Webhook] Decryption failed: %v", err)
		c.JSON(http.StatusBadRequest, gin.H{"error": "decryption failed"})
		return
	}

	// Verify HMAC-SHA256 signature
	// Sign params: eventData (decrypted), x-tm-app-key, x-tm-event-code, x-tm-ext-param, x-tm-timestamp
	if !temuVerifyWebhookSignature(appSecret, signature, map[string]string{
		"eventData":       decrypted,
		"x-tm-app-key":    appKey,
		"x-tm-event-code": eventCode,
		"x-tm-ext-param":  extParam,
		"x-tm-timestamp":  timestamp,
	}) {
		log.Printf("[Temu Webhook] Invalid signature for event %s", eventCode)
		c.JSON(http.StatusUnauthorized, gin.H{"error": "invalid signature"})
		return
	}
...
func (h *OrderWebhookHandler) getTemuAppSecret(ctx context.Context) string {
	if h.firestoreClient == nil {
		return os.Getenv("TEMU_APP_SECRET")
	}
...
// Params are sorted by key ascending, concatenated as key=value, then HMAC-SHA256.
