...
def scan_scorecard(skip=False):
    ...
    # Prefer GitHub remote URL if the repo is hosted there
    # Try to detect git remote
    rc_remote, remote_url, _ = run("git remote get-url origin", cwd=PLATFORM_DIR, timeout=10)
    is_github = rc_remote == 0 and "github.com" in remote_url
    ...
    if is_github:
        ...
        repo_ref = f"github.com/{m.group(1)}" if m else None
    else:
        repo_ref = None
    ...
    if repo_ref:
        ...
        if github_token:
            ...
            print("  ⚠ GITHUB_TOKEN not set — some checks may be limited.")
            print("  Set $env:GITHUB_TOKEN = 'ghp_...' for full results.")
            env_prefix = ""
        rc, stdout, stderr = run(f"{env_prefix}{cmd}", timeout=300)
    else:
        print(f"  No GitHub remote detected — running local filesystem checks.")
        print(f"  Scanning: {PLATFORM_DIR}")
        cmd = (f'scorecard --local="{PLATFORM_DIR}" --format=json --show-details '
               f'> "{out}" 2>&1')
