...
const btnStyle = (disabled = false, variant: 'primary' | 'secondary' | 'danger' = 'primary'): React.CSSProperties => ({
  padding: '9px 20px',
  background: disabled ? 'rgba(99,102,241,0.3)' :
    variant === 'danger' ? 'rgba(239,68,68,0.15)' :
    variant === 'secondary' ? 'var(--bg-elevated)' : 'var(--primary)',
  border: variant === 'secondary' ? '1px solid var(--border)' :
          variant === 'danger' ? '1px solid rgba(239,68,68,0.4)' : 'none',
  borderRadius: 8,
  color: disabled ? 'rgba(255,255,255,0.4)' :
    variant === 'danger' ? '#f87171' :
    variant === 'secondary' ? 'var(--text-primary)' : '#fff',
  ...
  return (
    <div style={{ padding: '32px 40px', maxWidth: 680, margin: '0 auto' }}>
      <div style={{ marginBottom: 28 }}>
        <h1 style={{ margin: 0, color: 'var(--text-primary)', fontSize: 22, fontWeight: 700 }}>👤 My Profile</h1>
        ...
        {phoneVerified ? (
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
              ...
              <button onClick={changePhone} style={btnStyle(false, 'secondary')}>Change</button>
            </div>
          </div>
        ) : (
          <div>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                {(['sms', 'whatsapp'] as const).map(ch => (
                  <button key={ch} onClick={() => setPhoneChannel(ch)} style={{
                    padding: '8px 18px', borderRadius: 8, fontWeight: 600, fontSize: 13,
                    border: `1px solid ${phoneChannel === ch ? 'var(--primary)' : 'var(--border)'}`,
                    background: phoneChannel === ch ? 'rgba(99,102,241,0.15)' : 'var(--bg-elevated)',
                    color: phoneChannel === ch ? 'var(--primary)' : 'var(--text-secondary)',
                    cursor: 'pointer',
                  }}>
                    {ch === 'sms' ? '💬 SMS' : '💚 WhatsApp'}
                  </button>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
                  ...
                  disabled={otpSent} />
                <button onClick={sendOtp} disabled={sendingOtp || !phone.trim()} style={btnStyle(sendingOtp || !phone.trim())}>
            ...
            {otpSent && (
              <div style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button onClick={verifyOtp} disabled={verifyingOtp || otp.length < 4} style={btnStyle(verifyingOtp || otp.length < 4)}>
      ...
      <div style={cardStyle}>
        ...
        <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 12 }}>
          <button onClick={saveNotifPrefs} disabled={savingNotif} style={btnStyle(savingNotif)}>
