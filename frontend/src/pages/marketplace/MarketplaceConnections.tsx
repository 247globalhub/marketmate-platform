...
function ChannelLogo({ id, size = 24, thumbnailUrl }: { id: string; size?: number; thumbnailUrl?: string }) {
  ...
  const color = ({ amazon:'#FF9900',temu:'#FF6B35',ebay:'#E53238',shopify:'#96BF48',tesco:'#00539F',amazon_vendor:'#232F3E',tiktok:'#010101',etsy:'#F1641E',woocommerce:'#7c3aed',walmart:'#0071ce',kaufland:'#e5002b',magento:'#f97316',bigcommerce:'#1C4EBF',onbuy:'#E76119',backmarket:'#14B8A6',zalando:'#FF6600',bol:'#0E4299',lazada:'#F57224' } as Record<string,string>)[id] || '#666';
...
export default function MarketplaceConnections() {
  ...
  const [configTab, setConfigTab] = useState<'orders' | 'stock' | 'shipping'>('orders');
  ...
  const THIRD_PARTY_ADAPTERS = new Set(['woocommerce', 'magento', 'bigcommerce', 'shopify']);
    ...
    <div className="page">
      <div className="page-header" style={{ marginBottom: 0, paddingBottom: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <button onClick={() => navigate('/marketplace/compare')} style={{
            ...
            <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
              {(['orders', 'stock', 'shipping'] as const).map(tab => (
                <button key={tab} onClick={() => setConfigTab(tab)} style={{ flex: 1, padding: '12px 0', background: 'none', border: 'none', borderBottom: configTab === tab ? '2px solid var(--primary)' : '2px solid transparent', color: configTab === tab ? 'var(--primary)' : 'var(--text-muted)', fontWeight: configTab === tab ? 700 : 400, fontSize: 13, cursor: 'pointer', textTransform: 'capitalize' }}>
                  {tab === 'orders' ? '📦 Orders' : tab === 'stock' ? '📊 Stock' : '🚚 Shipping'}
                </button>
              ))}
            </div>
