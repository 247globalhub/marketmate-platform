...
export default function ListingCreate() {
  ...
  return (
    <div className="page">
      ...
      {step === 2 && (
        <div className="card" style={{ padding: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, marginBottom: 6 }}>Choose marketplace channel</h3>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 20 }}>Listing: <strong>{selectedProduct?.title}</strong></p>
          {availableChannels.length === 0 ? (
            <div className="empty-state"><div className="empty-icon">🔗</div><h3>No connections available</h3><p>Connect a marketplace first.</p>
              <button className="btn btn-primary" onClick={() => navigate('/marketplace/connections')}>Connect Marketplace</button></div>
          ) : (
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12, marginBottom: 20 }}>
                {availableChannels.map(ch => {
                  const count = activeCreds.filter(c => c.channel === ch).length;
                  return (
                    <div key={ch} onClick={() => { setSelectedChannel(ch); setSelectedCredentialId(activeCreds.find(c => c.channel === ch)?.credential_id || ''); }}
                      style={{ padding: 20, borderRadius: 10, cursor: 'pointer', textAlign: 'center', background: selectedChannel === ch ? 'var(--primary-glow)' : 'var(--bg-tertiary)', border: `1px solid ${selectedChannel === ch ? 'var(--primary)' : 'var(--border-bright)'}` }}>
                      <div style={{ fontSize: 32, marginBottom: 8 }}>{adapterEmoji[ch] || '🌐'}</div>
                      <div style={{ fontWeight: 700, textTransform: 'capitalize' }}>{ch}</div>
                      <div style={{ fontSize: 11, color: 'var(--success)', marginTop: 4 }}>{count} account{count > 1 ? 's' : ''}</div>
                    </div>
                  );
                })}
              </div>
              <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
                ...
                <button className="btn btn-primary" onClick={handleContinue} disabled={!selectedChannel}>Continue →</button>
              </div>
            </>
          )}
        </div>
